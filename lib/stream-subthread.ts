...
export interface StreamSubThreadParams {
  subThreadId: string;
  model: string;
...
export interface StreamSubThreadResult {
  messageId: string;
  usage: { tokensIn: number; tokensOut: number };
  sections: ParsedSectionForDb[];
}

export async function streamSubThreadResponse(
  params: StreamSubThreadParams,
  write: (chunk: string) => void,
): Promise<StreamSubThreadResult> {
  const { subThreadId, model, parentSummary, sectionContent, messages } =
    params;
  ...
  const systemContext = `${SECTION_SYSTEM_PROMPT}

The following is a summary of the parent conversation (for context only). The user is asking a follow-up about a specific section.

--- Parent conversation summary ---
${parentSummary}

--- Section the user is asking about ---
${sectionContent}
---`;
  ...
  const apiMessages = [
    {
      role: "user" as const,
      content: "I have a question about the section above.",
    },
    ...messages.map((m) => ({
